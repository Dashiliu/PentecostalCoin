文章将首先介绍 Flink Runtime 的整体架构以及 Job 的基本执行流程，然后介绍在这个过程中 Flink 是怎么进行资源管理、作业调度以及错误恢复的。最后，本文还将简要介绍 Flink Runtime 层当前正在进行的一些工作。
...
图 2. Flink 集群的基本结构。Flink Runtime 层采用了标准的 master-slave 架构(*说明包括了RPC*)。
